...
SECRET_KEY_ENV = "KUIN_G_SECRET_KEY_B64"
SECRET_KEY_FILE_ENV = "KUIN_G_SECRET_KEY_FILE"
DEFAULT_SECRET_KEY_FILE = "secret.key"


def load_secret_key(key_file: Path | None = None, create_if_missing: bool = True) -> bytes:
    """Load the 256-bit HMAC secret key.

    Priority:
      1. KUIN_G_SECRET_KEY_B64 environment variable (base64 encoded)
      2. KUIN_G_SECRET_KEY_FILE environment variable
      3. secret.key in the application/data directory

    A 32-byte key is required. On first run, a cryptographically random 32-byte
    key is generated and stored with restrictive permissions where supported.
    """
    encoded = os.environ.get(SECRET_KEY_ENV, "").strip()
    if encoded:
        try:
            key = base64.b64decode(encoded, validate=True)
        except Exception as exc:
            raise ValueError(f"{SECRET_KEY_ENV} must be valid base64") from exc
        if len(key) != 32:
            raise ValueError(f"{SECRET_KEY_ENV} must decode to exactly 32 bytes (256 bits)")
        return key

    configured = os.environ.get(SECRET_KEY_FILE_ENV, "").strip()
    path = Path(configured) if configured else (key_file or Path(DEFAULT_SECRET_KEY_FILE))

    if path.exists():
        key = path.read_bytes()
        if len(key) != 32:
            raise ValueError(f"HMAC secret key {path} must be exactly 32 bytes (256 bits)")
        return key

    if not create_if_missing:
        raise FileNotFoundError(f"HMAC secret key not found: {path}")
...
def make_kuin(drone_id: object, secret_key: bytes | None = None) -> str:
    """Generate KUIN-G from the Drone ID using HMAC-SHA256.

    The Drone ID is the primary key/message. HMAC-SHA256 produces a 256-bit
    digest. Base32 encoding converts the digest to an alphanumeric alphabet
    (A-Z and 2-7), and the first 16 characters are used as the KUIN-G.
    """
    key = secret_key if secret_key is not None else load_secret_key()
    if len(key) != 32:
        raise ValueError("HMAC secret key must be exactly 32 bytes (256 bits)")
    message = str(drone_id).encode("utf-8")
...
def generate(
    ...
    kuin_values = [make_kuin(value, secret_key) for value in values]
...
def main() -> int:
    ...
    parser.add_argument(
        "--key-file",
        help="Path to the 32-byte HMAC secret key file.",
    )
    ...
    try:
        if args.key_file:
            os.environ[SECRET_KEY_FILE_ENV] = args.key_file
        generate(
