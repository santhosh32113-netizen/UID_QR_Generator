#!/usr/bin/env python3
"""Build a project-status PowerPoint from the KUIN-G Generator evidence."""
...
OUT = ROOT / "output" / "KUIN_G_Development_Metrics.pptx"
...
def title(slide, kicker, heading, number):
    ...
    text(slide, f"KUIN-G GENERATOR  /  {number:02d}", 11.55, 0.48, 1.25, 0.25, 8, MUTED, True, "Aptos", PP_ALIGN.RIGHT)
...
def build() -> None:
    ...
    text(slide, "KUIN-G Generator", 0.72, 1.82, 8.8, 0.78, 36, WHITE, True, "Aptos Display")
    ...
    metric(slide, 0.7, 1.95, "0", "records at baseline", TEAL)
    metric(slide, 3.7, 1.95, "16", "KUIN-G characters", RGBColor(103, 179, 150))
    metric(slide, 6.7, 1.95, "3", "QR backup formats", CORAL)
    metric(slide, 9.7, 1.95, "1", "Excel source of truth", GOLD)
    bullets(slide, [
        "Built a repeatable Drone ID and KUIN-G workflow around the source workbook.",
        "Added duplicate detection, hierarchy validation and authoritative server generation.",
        "Produced PNG, SVG and STL QR assets plus a data-driven local operations panel.",
    ...
    layers = [
        ...
        ("03", "KUIN-G + QR generation", "Create one KUIN-G and PNG, SVG and STL QR files per Drone ID.", GOLD),
        ("04", "Register continuity", "Rebuild controlled registers from the authoritative Excel workbook.", RGBColor(103, 179, 150)),
    ...
    metric(slide, 3.7, 1.95, "16-char", "KUIN-G payload", RGBColor(82, 136, 170))
    metric(slide, 6.7, 1.95, "100%", "input IDs mapped", RGBColor(103, 179, 150))
    metric(slide, 9.7, 1.95, "0", "duplicate KUIN-G collisions", CORAL)
    box(slide, 0.72, 3.62, 5.85, 2.36, MINT, MINT, True)
    text(slide, "VALIDATION COVERAGE", 1.0, 3.88, 3.8, 0.26, 9, TEAL, True)
    bullets(slide, ["Duplicate hierarchy IDs checked before save", "Server generates the primary Drone ID", "Every KUIN-G checked for PNG, SVG and STL files"], 1.0, 4.28, 5.0, 1.4, 14)
    ...
    text(slide, "deterministic KUIN-G", 1.12, 4.25, 2.9, 0.42, 22, WHITE, True, "Aptos Display")
    ...
    bullets(slide, ["Server-generated Drone ID", "16-character KUIN-G generation", "Duplicate and hierarchy safeguards", "PNG, SVG and STL QR distribution", "Data-driven interactive panel", "Power BI-ready CSV export"], 1.1, 2.72, 4.8, 2.4, 15)
    ...
    text(slide, "Success measure: trusted KUIN-G identity plus fast operational filtering.", 0.88, 6.38, 11.4, 0.34, 18, NAVY, True, "Aptos Display", PP_ALIGN.CENTER)
